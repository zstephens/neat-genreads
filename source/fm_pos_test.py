...
ploidy = 2
start_time = time()
temp_symbol_string = CigarStringNew('10777M1I322M').string_to_list()
...
print(f'temp_symbol_string = {temp_symbol_string}')
print(f'temp_symbol_string2 = {temp_symbol_string2}')
assert(len(temp_symbol_string) == len(temp_symbol_string2))
print('passed')
# assert(temp_symbol_string == temp_symbol_string2)
# print('passed')
...
for i in range(ploidy):
    ...
    for j in range(len(temp_symbol_string)):
        ...
        if temp_symbol_string[j] == 'D':
            fm_pos2[i][-1] += temp_symbol_string[j].count('D')
        # compute number of ref matches for each read
        span_dif = len([n for n in temp_symbol_string[j:j + read_len] if n == 'M'])
