...
class CigarString:
    ...
    def __add__(self, other):
        """
        This adds functionality to the "+" operation that makes sure it returns a CigarString object.
        Now you can add two CigarStrings together and get a CigarString back in return.
        :param other: Another object
        :return: CigarString or a list

        >>>my_cigar = CigarString('100M')
        >>>pi = 30
        >>>pf = 32
        >>>new_cig = my_cigar[:pi + 1] + my_cigar[pf:]
        >>>assert(len(new_cig) == 99)
        """
        self._needs_update = True
        if isinstance(other, CigarString):
            result = self._cigar.__add__(other.cigar)
        else:
...
if __name__ == '__main__':
    ...
    new_cig = my_cigar[:5] + ["I"] * 5 + my_cigar[5:]
    print(new_cig.cigar)
