...
def main():
    ...
    parser.add_argument('-bi', type=str, required=False, metavar='<str>', default=None,
                        help="only_use_these_regions.bed")
    parser.add_argument('-be', type=str, required=False, metavar='<str>', default=None,
    ...
    # parser.add_argument('--no-whitelist', required=False, action='store_true', default=False,
    #                     help='allow any non-standard ref')
    ...
    # (ref, tsv, out_pickle, save_trinuc, no_whitelist, skip_common) = (
    #     args.r, args.m, args.o, args.save_trinuc, args.no_whitelist, args.skip_common)
    ...
    ref_inds = SeqIO.to_dict(SeqIO.parse(ref, "fasta"))

    # simplify naming
    for key in ref_inds.keys():
        splt = key.split("|")
        ref_inds[splt[0]] = ref_inds.pop(key)

    ref_list = list(ref_inds.keys())
    ...
    for ref_name in ref_list:
        ...
        # if (ref_name not in REF_WHITELIST) and (not no_whitelist):
        #     print(ref_name, 'is not in our whitelist, skipping...')
        #     continue

        # Count the number of non-N nucleotides for the reference
        TOTAL_REFLEN += len(ref_inds[ref_name].seq) - ref_inds[ref_name].seq.count('N')
        ...
        if mybed is not None:
            ...
            if refKey in mybed[0]:
                subRegions = [(mybed[0][refKey][n], mybed[0][refKey][n + 1]) for n in
                              range(0, len(mybed[0][refKey]), 2)]
                for sr in subRegions:
                    for i in range(sr[0], sr[1] + 1 - 2):
                        trinuc = ref_inds[ref_name][i:i + 3]
        ...
        if not os.path.isfile(ref + '.trinucCounts'):
            ...
            for i in range(len(ref_inds[ref_name]) - 2):
                if i % 1000000 == 0 and i > 0:
                    print(i, '/', len(ref_inds[ref_name]))
                    # break
                trinuc = ref_inds[ref_name][i:i + 3]
                if not trinuc in VALID_TRINUC:
        ...
        for line in f:
            ...
            if isFirst:
                if is_vcf:
                    ...
                    splt = line.strip().split('\t')
                    (c1, c2, c3) = (
                        splt.index('chromosome'), splt.index('chromosome_start'), splt.index('chromosome_end'))
                    (m1, m2, m3) = (splt.index('reference_genome_allele'), splt.index('mutated_from_allele'),
                                    splt.index('mutated_to_allele'))
            ...
            if ('-' not in allele_normal and '-' not in allele_tumor) and (
                    len(allele_normal) > 1 or len(allele_tumor) > 1):
            ...
            if '-' not in allele_normal and '-' not in allele_tumor and chrStart == chrEnd:
                trinuc_ref = ref_inds[ref_name][chrStart - 1:chrStart + 2]
                ...
                if allele_ref == trinuc_ref[1]:
                    trinuc_normal = ref_inds[ref_name][chrStart - 1] + allele_normal + ref_inds[ref_name][chrStart + 1]
                    trinuc_tumor = ref_inds[ref_name][chrStart - 1] + allele_tumor + ref_inds[ref_name][chrStart + 1]
        ...
        for n in byLen:
            ...
            candidate_regions.append((n[0] / float(bf - bi), max([0, bi]), min([len(ref_inds[ref_name]), bf])))
