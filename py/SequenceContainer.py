...
class SequenceContainer:
    def __init__(self, x_offset, sequence, ploidy, window_overlap, read_len, mut_models=[], mut_rate=None, only_vcf=False):
        # initialize basic variables
        self.only_vcf = only_vcf
        self.init_basicVars(x_offset, sequence, ploidy, window_overlap, read_len)
        # initialize mutation models
        self.init_mutModels(mut_models, mut_rate)
    ...
    def init_basicVars(self, x_offset, sequence, ploidy, window_overlap, read_len):
        self.x         = x_offset
        self.ploidy    = ploidy
        ...
        self.blackList = [np.zeros(self.seq_Len, dtype='<i4') for n in range(self.ploidy)]

        # disallow mutations to occur on window overlap points
        self.winBuffer = window_overlap
        ...
        if not self.only_vcf:
            ...
            for i in range(len(self.sequences)):
                ...
                while j+self.window_size < len(self.sequences[i]):
                    gc_c = self.sequences[i][j:j+self.window_size].count('G') + self.sequences[i][j:j+self.window_size].count('C')
                    gcCov_vals[i].extend([gc_scalars[gc_c]]*self.window_size)
                    j += self.window_size
                gc_c = self.sequences[i][-self.window_size:].count('G') + self.sequences[i][-self.window_size:].count('C')
                ...
                for j in range(1,max_coord):
                    if self.FM_pos[i][j] is None:
                        ...
                        tr_cov_vals[i].append(sum(target_cov_vals[self.FM_pos[i][j]:self.FM_span[i][j]])/float(self.FM_span[i][j]-self.FM_pos[i][j]))
                ...
                #
                covvec = np.cumsum([tr_cov_vals[i][nnn]*gcCov_vals[i][nnn] for nnn in range(len(tr_cov_vals[i]))])
                coverage_vals = []
                for j in range(0,max_coord):
                    coverage_vals.append(covvec[j+self.read_len] - covvec[j])
                    ...
                    for flv in sorted(list(set(self.fraglens_ind_map.values()))):
                        ...
                        for j in frag_dist.values:
                            if self.fraglens_ind_map[j] == flv and j > buffer_val:
                                buffer_val = j
                        max_coord = min([len(self.sequences[i])-buffer_val-1, len(self.all_cigar[i])-buffer_val+self.read_len-2])
                        ...
                        coverage_vals = []
                        for j in range(0,max_coord):
                            coverage_vals.append(covvec[j+self.read_len] - covvec[j] + covvec[j+flv] - covvec[j+flv-self.read_len])
    ...
    def init_mutModels(self,mut_models,mut_rate):
        if mut_models == []:
            ml = [copy.deepcopy(DEFAULT_MODEL_1) for n in range(self.ploidy)]
            self.model_data = ml[:self.ploidy]
        else:
        ...
        for n in self.model_data:
            self.models.append([self.mut_scalar*n[0], n[1], n[2], n[3], DiscreteDistribution(n[5], n[4]), DiscreteDistribution(n[7], n[6]), []])
            for m in n[8]:
    ...
    def init_poisson(self):
        ind_l_list = [self.seq_Len*self.models[i][0]*self.models[i][2]*self.ploid_mut_frac[i] for i in range(len(self.models))]
        snp_l_list = [self.seq_Len*self.models[i][0]*(1.-self.models[i][2])*self.ploid_mut_frac[i] for i in range(len(self.models))]
    ...
    def init_trinucBias(self):
        ...
        for p in range(self.ploidy):
            for i in range(self.winBuffer+1,self.seq_Len-1):
                trinuc_snp_bias[p][i] = self.models[p][7][ALL_IND[str(self.sequences[p][i-1:i+2])]]
            self.trinuc_bias[p] = DiscreteDistribution(trinuc_snp_bias[p][self.winBuffer+1:self.seq_Len-1],range(self.winBuffer+1,self.seq_Len-1))
    ...
    def insert_mutations(self, inputList):
        for inpV in inputList:
            ...
            if wps == None:	# if no genotype given, assume heterozygous and choose a single ploid based on their mut rates
                whichPloid.append(self.ploid_mut_prior.sample())
                whichAlt = [0]
            else:
                if '/' in wps or '|' in wps:
                    if '/' in wps:
        ...
        for i in range(self.ploidy):
            for j in range(self.indelsToAdd[i]):
                ...
                for attempt in range(MAX_ATTEMPTS):
                    eventPos = random.randint(self.winBuffer,self.seq_Len-1)
                    for p in whichPloid:
                ...
                skipEvent = False
                if eventPos+len(myIndel[1]) >= self.seq_Len-self.winBuffer-1:
                    skipEvent = True
                if skipEvent:
                    continue
                for p in whichPloid:
        ...
        for i in range(self.ploidy):
            for j in range(self.snpsToAdd[i]):
                ...
                for attempt in range(MAX_ATTEMPTS):
                    ...
                    if IGNORE_TRINUC:
                        eventPos = random.randint(self.winBuffer+1,self.seq_Len-2)
                    else:
        ...
        for i in range(len(all_indels_ins)):
            ...
            for j in range(len(all_indels_ins[i])):
                ...
                if all_indels_ins[i][j][1] != str(self.sequences[i][v_pos:v_pos2]):
                    ...
                    if d > 0:
                        temp_symbol_string = temp_symbol_string[:v_pos+1] + ['I']*d + temp_symbol_string[v_pos2+1:]
                    elif d < 0:
    ...
    def sample_read(self, sequencingModel, frag_len=None):
        ...
        for r in readsToSample:
            ...
            r[3] += self.sequences[my_ploid][r[0]+self.read_len:r[0]+self.read_len+totalD]
            expanded_cigar = []
            extraCigar    = []
            adj           = 0
                ...
                if error[0] == 'D' or error[0] == 'I':
                    ...
                    if error[0] == 'D':
                        ...
                        if str(r[3][pi:pf]) == str(error[3]):
                            ...
                            if pi+1 == len(expanded_cigar):	# weird edge case with del at very end of region. Make a guess and add a "M"
                                expanded_cigar.append('M')
                            expanded_cigar[pi+1] = 'D'*eLen + expanded_cigar[pi+1]
                        else:
                    ...
                    # insert insertion error into read and update cigar string accordingly
                        ...
                        if str(r[3][ePos+myadj]) == error[3]:
                            ...
                            expanded_cigar = expanded_cigar[:ePos+myadj] + ['I']*eLen + expanded_cigar[ePos+myadj:]
                        else:
